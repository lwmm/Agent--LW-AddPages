...
class CreateHierachicPageStructure
{
	...
	{
		if ($check) echo "<pre>"; print_r($array); echo "</pre>";
		$j=1;
		...
		foreach ($array as $page)
		{
			...
			{
				if (strlen(trim($pathpart))>0) $path.= $phash[$pathpart].":";
			...
			if (strlen(trim($name))>0)
			{
				...
				if (!$error)
				{
					...
					{
						$phash[$temp_id] = $this->commandHandler->addPage($data);
					}
				}
				else 
				{
        ...
        \AgentAddPages\Services\Page::reload($baseUrl.$pid);
	}	
}